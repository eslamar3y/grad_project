...
export default function RemoveEquipment({ onClose, showRemoveModal, onRemove, selectedEquipment }) {
        ...
        <Modal open={showRemoveModal} onClose={onClose}>
            <div className="px-6 py-6 rounded-2xl bg-mainColor relative">
                ...
                <div className=" border-t-2 flex gap-4 justify-end pt-3">
                    <button onClick={onClose} className="text-secondColor outline-none font-semibold">Cancel</button>
                    <button onClick={handleRemove} className="text-secondColor hover:text-red-600 outline-none font-semibold">Remove</button>
                </div>
            </div>
        </Modal>
    )
